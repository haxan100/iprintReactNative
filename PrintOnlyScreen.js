...
const PrintOnlyScreen = ({ navigation,notificationCount }) => {
  ...
  launchImageLibrary(options, (response) => {
    console.log("Response: ", response); // Debug log
      if (response.didCancel) {
        ...
        setImage({ uri: uri });
        navigation.navigate('Preview', { image: uri }); // Kirimkan URI sebagai string
      } else {
  ...
  const pickImage = async () => {
    ...
    ImagePicker.showImagePicker({
      ...
      if (response.didCancel) {
        ...
        navigation.navigate('Preview', { imageUri: source.uri });
      }
    });
  };
  return (
